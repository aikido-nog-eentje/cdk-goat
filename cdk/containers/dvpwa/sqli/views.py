...
async def student(request: Request):
    ...
    courses_marks = {c: list(ms) for c, ms in groupby(marks, lambda m: m.course_id)}
    results = [
...
async def courses(request: Request):
    ...
    if request.method == "POST":
        # Only authenticated admin users can create courses
...
async def evaluate(request: Request):
    ...
    async with app["db"].acquire() as conn:
        ...
        try:
            ...
            return {"errors": e.as_dict(), "course": course, "student": student}
